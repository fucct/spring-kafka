...
When using manual `AckMode`, you can also provide the listener with the `Acknowledgement`.
...
public void listen(String data, Acknowledgement ack) {
    ...
    ack.acknowledge();
}
----
...
Alternatively, you can receive a `List` of `Message<?>` objects with each offset and other details in each message, but it must be the only parameter (aside from optional `Acknowledgement`, when using manual commits, and/or `Consumer<?, ?>` parameters) defined on the method.
The following example shows how to do so:
...
public void listen2(List<Message<?>> list, Acknowledgement ack) {
    ...
}

@KafkaListener(id = "listMsgAckConsumer", topics = "myTopic", containerFactory = "batchFactory")
public void listen3(List<Message<?>> list, Acknowledgement ack, Consumer<?, ?> consumer) {
    ...
}
----
...
You can also receive a list of `ConsumerRecord<?, ?>` objects, but it must be the only parameter (aside from optional `Acknowledgement`, when using manual commits and `Consumer<?, ?>` parameters) defined on the method.
The following example shows how to do so:
...
public void listen(List<ConsumerRecord<Integer, String>> list, Acknowledgement ack) {
    ...
}
----
...
Again, this must be the only parameter (aside from optional `Acknowledgement`, when using manual commits or `Consumer<?, ?>` parameters) on the method.
The following example shows how to do so:
