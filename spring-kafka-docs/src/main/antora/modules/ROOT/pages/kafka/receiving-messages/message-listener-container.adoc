...
* `MANUAL`: The message listener is responsible to `acknowledge()` the `Acknowledgement`.
After that, the same semantics as `BATCH` are applied.
* `MANUAL_IMMEDIATE`: Commit the offset immediately when the `Acknowledgement.acknowledge()` method is called by the listener.
...
The `Acknowledgement` has the following method:
...
Starting with version 2.3, the `Acknowledgement` interface has two additional methods `nack(long sleep)` and `nack(int index, long sleep)`.
...
Starting with version 3.0.10, batch listeners can commit the offsets of parts of the batch, using `acknowledge(index)` on the `Acknowledgement` argument.
